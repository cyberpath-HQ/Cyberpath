...
import { type SearchableBlogPost, FUSE_OPTIONS } from "@/lib/search-utils";
import { ALL_CATEGORIES, getCategoryColor, type Category } from "@/lib/categories";
...
export function BlogList({
  ...
  const filteredPosts = useMemo(() => {
    ...
    return result;
  }, [posts, fuse, searchQuery, selectedCategory, selectedTags]);

  // Pagination
  const totalPages = useMemo(() => Math.ceil(filteredPosts.length / itemsPerPage), [filteredPosts, itemsPerPage]);
                  ...
                  <CardHeader>
                    <div className="mb-2 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <AuthorAvatar name={author.name} avatarSrc={author.avatar?.src} className="h-8 w-8">
                          <OptimizedImage
